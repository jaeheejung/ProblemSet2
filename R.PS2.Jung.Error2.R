...
#NOTE: The part of benfords() that creates an error is commented by "##". But because the same code chunk in benfords() determines the Benford's distribution regardless of when dataset 1 or dataset 2 is put in, if the function calculates the wrong Benford's distribution for dataset 1, it calculates the wrong Benford's distribution for dataset 2, and vice versa. That is, the function does not create an error only for dataset 1 or dataset 2. To make up for this, I will create two edited versions of the code for benfords() that miscalculate the Benford's distribution. The code in this file is the second of the two edited versions.
...
benfords <- function(votes.tot,statistic){
	...
	for(i in 1:9){
		prop.freq[i] <- sum((substr(votes.tot,start=1,stop=1)==1)*1)/length(votes.tot)
		##For loop is not being done properly, because there is a typo of 1, which should have been i, after ==
