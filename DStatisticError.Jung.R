...
# NOTE: This file contains code for benfords() that would fail to pass the unit test by miscalculating the d statistic. This code generates error for both dataset 1 and dataset 2 because the same code chunk in benfords() determines how the d statistic is calculated regardless of when dataset 1 or dataset 2 is put in. That is, if the function calculates the wrong d statistic for dataset 1, it calculates the wrong d statistic for dataset 2, and vice versa. So if we run the unit test on the following function, we will see errors for both dataset 1 and dataset 2. The part of benfords() that creates error is commented by "##".
...
benfords <- function(votes.tot,statistic){
	...
	if(statistic=="both"){ #When you want both m and d statistics
		...
		d <- numeric(9)
		for(i in 1:9){
			d[i] <- (prop.freq[i]-log(1+(1/i)))^2
		##There is a typo, because log indicates ln, not log10.
		}
