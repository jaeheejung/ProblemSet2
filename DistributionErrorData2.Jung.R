...
# NOTE: This file contains code for benfords() that would fail to pass the unit test under dataset 2, which is bad data that does not meet Benford's law. In particular, the code provokes an error about the Benford's distribution for dataset 2. The part of benfords() that creates an error is commented by "##".
# But it must be noted that because the same code chunk in benfords() determines how the Benford's distribution is calculated regardless of when dataset 1 or dataset 2 is put in, if the function calculates the wrong Benford's distribution for dataset 2, it calculates the wrong Benford's distribution for dataset 1, and vice versa. That is, if we run the unit test on the following function, we will see errors for both dataset 1 and dataset 2. Moreover, because the code for calculating the Benford's distribution is used in calculating both m and d statistics, if there is an error in the code for calculating the distribution, we see errors for all other miscalculations. This, however, does not matter, because when all errors come up, we know that the underlying algorithm of all calculations--the Benford's distribution--is incorrect.
...
benfords <- function(votes.tot,statistic){
	...
	for(i in 1:9){
		...
		##The calculation for proportional frequency is wrong, because you have to divide by the length of the dataset, not 9.
